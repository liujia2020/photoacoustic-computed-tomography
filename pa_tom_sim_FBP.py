...
def fbp(prec, xd, t, zTargs, c=1484):
    ...
    freq = fs/2*np.linspace(0,1,int(NFFT/2+1))

    p_filtered = np.zeros_like(prec)
    for xi in range(len(xd)):
        for yi in range(len(xd)):
            p_freq = fft(prec[:,xi,yi], NFFT)
            ramp_filter = 2 * np.pi * freq  # ramp filter
            p_freq_filtered = p_freq[:int(NFFT/2+1)] * ramp_filter  # apply filter
            p_filtered[:,xi,yi] = np.real(ifft(np.concatenate((p_freq_filtered, np.conj(p_freq_filtered[-2:0:-1]))), NFFT))[:len(t)]

    p0 = np.zeros((len(xf), len(yf), len(zf))) 
    for xi in range(len(xd)):
        X2 = (Xf-xd[xi])**2  
        for yi in range(len(xd)):
            dist = np.sqrt(X2 + (Yf-xd[yi])**2 + Zf2)
            distind = np.round(dist*(fs/c)).astype(int)
            p0 += p_filtered[distind, xi, yi] * Zf / dist
        print('Reconstructing image with detector row', len(xd)-xi)

    p0 /= (2 * np.pi)
    p0max = p0.max()
    p0norm = p0 / p0max
    return p0norm, xf, yf, zf

def fbp3d(prec, xd, t, zl, zh, c=1484):
    res = 500e-6  # desired spatial resolution (m)
    xf = arange2(xd[0], xd[-1]+res, res)  # (m) choose reconstruction space to be directly above detector array
    yf = xf.copy()  
    zf = arange2(zl*1e-3, zh*1e-3+res, res)  # z location of targets
    Yf, Xf, Zf = np.meshgrid(yf, xf, zf)
    Zf2 = Zf**2
...
if __name__ == '__main__':
    ...
    if args.volumetric:
        pfnorm, xf, yf, zf = fbp3d(sigs, xd, t, args.range[0],args.range[1])
        tomPlot3D(pfnorm, xf, yf, zf, 6)
    if args.planar:
        pfnorm, xf, yf, zf = fbp(sigs, xd, t, args.targetPlane*1e-3)
        tomPlot2D(pfnorm, xf, yf, 6)
    if not args.volumetric and not args.planar:
        pfnorm, xf, yf, zf = perfTomog(sigs, xd, t, zTargs*1e-3)
        tomPlot2D(pfnorm, xf, yf, 6)
