...
def perfTomogOSEM(prec, xd, t, zTargs, niter=10, nsubset=10, c=1484):
    res = 500e-6  # desired spatial resolution (m)
    xf = arange2(xd[0],xd[-1]+res,res)  # (m) reconstruction space 
    yf = xf.copy() 
    zf = np.array([zTargs])
    Yf, Xf, Zf = np.meshgrid(yf, xf, zf)
    Zf2 = Zf**2
    ...
    rho = np.ones(Xf.shape)  # initialize reconstruction
    
    for i in range(niter):
        print(f"Iteration {i+1}/{niter}")
        for j in range(nsubset):
            subset_inds = range(j, len(xd), nsubset)
            proj = np.zeros(Xf.shape)
            
            for xi in subset_inds:
                X2 = (Xf-xd[xi])**2
                for yi in range(len(xd)):
                    dist = np.sqrt(X2+(Yf-xd[yi])**2+Zf2)
                    omega = (ds/dist**2)*Zf/dist
                    proj += omega*np.sum(prec[:,xi,yi])

            rho_new = rho * proj / np.sum(proj)
            rho_new[rho_new<1e-6] = 1e-6  # 避免过小的值
            rho = rho_new

    pfnorm = rho / np.max(rho)
    return pfnorm, xf, yf, zf

def perfTomog3dOSEM(prec, xd, t, zl, zh, niter=10, nsubset=10, c=1484):
    res = 300e-6  # desired spatial resolution (m)
    xf = arange2(xd[0],xd[-1]+res,res)  # (m) reconstruction space
    yf = xf.copy()
    zf = arange2(zl*1e-3,zh*1e-3+res,res)
    Yf, Xf, Zf = np.meshgrid(yf, xf, zf)
    Zf2 = Zf**2
    ...
    for i in range(niter):
        ...
        for j in range(nsubset):
            ...
            denom = ds*np.sum(omega,axis=(1,2))[:,None,None] 
            denom[denom==0] = 1  # 避免除0
            ratio = proj / (rho*denom)
            ratio[ratio==0] = 1  # 避免0次幂
            rho = rho * ratio**(omega/denom)
            rho[rho<1e-6] = 1e-6  # 避免过小的值
        ...
        for j in range(nsubset):
            ...
            for xi in subset_inds:
                ...
                for yi in range(len(xd)):  
                    dist = np.sqrt(X2+(Yf-xd[yi])**2+Zf2)
                    omega = (ds/dist**2)*Zf/dist
                    proj += omega*np.sum(prec[:,xi,yi])  # 累加prec在时间维度上的值
            
            ratio = proj / (rho*ds*np.sum(omega,axis=(1,2))[:,None,None])
            rho = rho * ratio**(omega/(ds*np.sum(omega,axis=(1,2))[:,None,None]))
...
if __name__ == '__main__':
    ...
    detPitch = (2.0/3.0)*1e-3 #step size when scanning individual detector 
    ...
    if args.volumetric:
        pfnorm, xf, yf, zf = perfTomog3dOSEM(sigs, xd, t, args.range[0],args.range[1]) 
        tomPlot3D(pfnorm, xf, yf, zf, 6)
    if args.planar:
        pfnorm, xf, yf, zf = perfTomogOSEM(sigs, xd, t, args.targetPlane*1e-3)
        tomPlot2D(pfnorm, xf, yf, 6)  
    if not args.volumetric and not args.planar:
        pfnorm, xf, yf, zf = perfTomogOSEM(sigs, xd, t, zTargs*1e-3)
        tomPlot2D(pfnorm, xf, yf, 6)
